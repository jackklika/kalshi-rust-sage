...
impl Kalshi {
    ...
    fn auth_headers(&self, path: &str) -> HeaderMap {
        ...
        match &self.auth {
            KalshiAuth::ApiKey { key_id, key, ..} => {
                ...
                let api_headers = api_key_headers(key_id, &mut signer, path, Method::GET).unwrap();
            ...
            KalshiAuth::EmailPassword => {
                headers.insert(HeaderName::from_static("Authorization"), HeaderValue::from_str(&self.curr_token.clone().unwrap()).unwrap());
            }
        }
    ...
    pub async fn http_get<T: DeserializeOwned>(&self, url: Url) -> Result<T, KalshiError> {
        self
            .client
            .get(url.clone())
            .headers(self.auth_headers(url.path()))
            .send()
            .await?
